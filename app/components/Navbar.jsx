    ...
    return (
        <>
            ...
            <nav className={`w-full fixed px-5 lg:px-8 xl:px-[8% py-4 flex items-center justify-between z-50 ${isScroll ? "bg-white bg-opacity-50 backdrop-blur-lg shadow-sm dark:bg-darkTheme dark:shadow-white/20" : ""}`}>
                <a href="#top" className="text-4xl font-bold mr-14">
                    ADJY.
                </a>
                ...
                <ul className={`hidden md:flex items-center gap-6 lg:gap-8 rounded-full px-12 py-3 ${isScroll ? "" : "bg-white shadow-sm bg-opacity-50 dark:border dark:border-white/50 dark:bg-transparent"} `}>
                    ...
                    <li><a className="font-Ovo" href="/bloglist">Blog</a></li> {/* Update Blog menu item */}
                </ul>
                ...
                <ul ref={sideMenuRef} className="flex md:hidden flex-col gap-4 py-20 px-10 fixed -right-64 top-0 bottom-0 w-64 z-50 h-screen bg-rose-50 transition duration-500 dark:bg-darkHover dark:text-white">
                    <div className="absolute top-6 right-6" onClick={closeMenu}>
                        <Image src={isDarkMode ? assets.close_white : assets.close_black} alt="" className="cursor-pointe w-6" />
                    </div>
                    ...
                    <li><a className="font-Ovo" onClick={closeMenu} href="/bloglist">Blog</a></li> {/* Update Blog menu item */}
                </ul>
            </nav>
        </>
    );
